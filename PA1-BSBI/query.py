...
def read_posting(term_id):
  ...
  posting_list = index_f.readline().strip().split()
...
while True:
  ...
  for each_query in input_parts[1:]:
      ...
      merged_posting = merge_posting(posting,merged_posting)
  # don't forget to convert doc_id back to doc_name, and sort in lexicographical order
  # before printing out to stdout
  merged_posting = [doc_id_dict[int(each)] for each in merged_posting]
  merged_posting.sort()
  outp = open('query_results','w')
