...
def validParts():
  ...
  partNames = ['Task 1 (Cosine Similarity)', \
                'Task 2 (BM-25)', \
...
def output(partId, ch_aux):
  ...
  if partId == 1:
    ...
    start = time()
    child = Popen(['./rank.sh', '1', 'queryDocTrainData', 'queryDocTrainRel', test_file.name], stdout=PIPE, stderr=PIPE, shell=False);
    ...
    if (len(guesses) != linesOutput):
      ...
      start = time()
      child = Popen(['./rank.sh', '2', 'queryDocTrainData', 'queryDocTrainRel', test_file.name], stdout=PIPE, stderr=PIPE, shell=False);
      (res, err) = child.communicate("")
      elapsed = time() - start
      guesses = res.splitlines()
      print err
      ...
      start = time()
      child = Popen(['./rank.sh', '3', 'queryDocTrainData', 'queryDocTrainRel', test_file.name], stdout=PIPE, stderr=PIPE, shell=False);
      (res, err) = child.communicate("")
      elapsed = time() - start
      guesses = res.splitlines()
      print err
